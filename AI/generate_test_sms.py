...
def generate_sms(scenario):
    ...
    upi_id = f"1234567890{random.randint(1, 9)}@upi"
    ref_num = f"1{random.randint(10000000000, 99999999999)}"
    
    date_str = get_random_date()
    # Formatting styles seen in real SMS
...
if __name__ == "__main__":
    print("-" * 60)
    print(" 💳 UPI ASSISTANT - TEST TRANSACTION GENERATOR 💳")
    print("-" * 60)
    ...
    for key, desc in scenarios:
        print(f"\n{desc}")
        print("► " + generate_sms(key))
    
    print("\n" + "-" * 60)
    print("Paste any of the above messages into the Transaction Analyzer in the Web UI!")
    print("-" * 60)
